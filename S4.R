...
for(s in 37:100){
  ...
  tmean<-tapply(mbm[,2],mbm[,1],mean)
  lmbm[s]<-32049697098/tmean['Safe_ENCox']
}

lambm<-data.frame(lam_radio,lmbm)
lambm<-lambm[-c(1:36),]

colnames(lambm)<-c('lam_ratio','speed_up')

pmbm<-ggplot(lambm)+geom_line(aes(x=lam_ratio, y = speed_up),size=1,color="blue")
pmbm+labs(x=expression(frac(lambda,lambda[max])),y='Speed up')