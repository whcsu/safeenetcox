...
mydata <- function(){
  #datasetname
  datasetname="GSE26712"
  data(GSE26712_eset, package = "curatedOvarianData")
  
  mypdata=phenoData(GSE26712_eset)[,c(4,5,7,8,9,10,12,18,19,24:27)]
  
  
  #change the covariates order
  mypdata=mypdata[ ,colnames(mypdata)[c(8,9,1:7,10:13)]]
  colnames(mypdata)[1] <- "time"
  colnames(mypdata)[2] <- "status"
  
  # log 2 transform is not needed
  tm=ExpressionSet(assayData=(exprs(GSE26712_eset)),phenoData=mypdata)
  ...
  ptr$status=as.numeric(ptr$status=="deceased")
  ptr[,c(3:6,8,10:13)]=lapply(ptr[,c(3:6,8,10:13)],as.factor)
  
  
  
  ptr=lapply(ptr, as.numeric)
  mydata=data.frame(ptr,t(exprs(tm)))
  # delete all columns with the same value
  mydata=mydata[vapply(mydata, function(x) length(unique(x)) > 1, logical(1L))]
  ...
  return (list(curdata=mydata,currii=rii,curdatasetname=datasetname))
}

Gdata<-mydata()
data<-Gdata$curdata[,-c(3,4,5)]
